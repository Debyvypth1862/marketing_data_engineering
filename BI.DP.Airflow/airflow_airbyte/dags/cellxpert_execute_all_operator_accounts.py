...
dag_id="CellxpertExecuteAllOperatorAccounts"
...
def check_dbt_dag_running(**context):
    ...
    dbt_dag_id = "CellxpertDbtTriggered"
...
with DAG(
    ...
    for res in result:
        ...
        task_name = f'process_operator_{operator_id_tuple[0][0]}.{replace_special_characters(f"S1_ Cellxpert {res[2]}_{res[1]}_{res[3]}")}'
        task_group_name = f"process_operator_{res[3]}"
        stage_1 = replace_special_characters(f"S1_ Cellxpert {res[2]}_{res[1]}_{res[3]}")
    ...
    trigger_cellxpert_dbt = TriggerDagRunOperator(
        task_id="trigger_cellxpert_dbt",
        trigger_dag_id="CellxpertDbtTriggered",
    ...
    end_task_group >> Jira_create >> end >> check_downstream_tasks >> check_dbt_enabled >> check_dbt_running >> trigger_cellxpert_dbt
