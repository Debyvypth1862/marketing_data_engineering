...
og_platform = constants.Inhouse
is_reprocess=False
platform_id = fetch_platform_id_from_platform(og_platform)
result = fetch_connid_oppid_by_platform(platform_id)
total_task_groups = len(result)
dag_id="BuffaloPartnersExecuteAllOperatorAccounts"
...
try:
    batch_size = int(Variable.get(f"{og_platform} batch_size"))
except:
...
with DAG(
    ...
    for res in result:
        ...
        task_name = f'process_operator_{operator_id_tuple[0][0]}.{replace_special_characters(f"S1_ Buffalo Partners {res[2]}_{res[1]}_{res[3]}")}'
        task_group_name = f"process_operator_{res[3]}"
        stage_1 = replace_special_characters(f"S1_ Buffalo Partners {res[2]}_{res[1]}_{res[3]}")
    ...
    check_dbt_running = ShortCircuitOperator(
        ...
        op_kwargs={"dag_id_to_check": "BuffaloPartnersDbtTriggered"},
        provide_context=True,
    )

    # Trigger DBT DAG after main processing completes
    trigger_buffalo_partners_dbt = TriggerDagRunOperator(
        task_id="trigger_buffalo_partners_dbt",
        trigger_dag_id="BuffaloPartnersDbtTriggered",
        conf={'triggered_by': 'main_dag'},
        wait_for_completion=False,
    )

    end >> check_downstream_tasks >> check_dbt_enabled >> check_dbt_running >> trigger_buffalo_partners_dbt
