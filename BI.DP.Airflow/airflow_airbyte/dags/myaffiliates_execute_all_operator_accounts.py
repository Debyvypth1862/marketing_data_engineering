...
dag_id="MyAffiliatesExecuteAllOperatorAccounts"
...
with DAG(
    ...
    for res in result:
        ...
        task_name = f'process_operator_{operator_id_tuple[0][0]}.{replace_special_characters(f"S1_ MyAffiliates {res[2]}_{res[1]}_{res[3]}")}'
        task_group_name = f"process_operator_{res[3]}"
        stage_1 = replace_special_characters(f"S1_ MyAffiliates {res[2]}_{res[1]}_{res[3]}")
    ...
    check_dbt_running = ShortCircuitOperator(
        ...
        op_kwargs={"dag_id_to_check": "MyAffiliatesDbtTriggered"},
        provide_context=True,
    )

    # Trigger DBT DAG after main processing completes
    trigger_myaffiliates_dbt = TriggerDagRunOperator(
        task_id="trigger_myaffiliates_dbt",
        trigger_dag_id="MyAffiliatesDbtTriggered",
    ...
    end >> check_downstream_tasks >> check_dbt_enabled >> check_dbt_running >> trigger_myaffiliates_dbt
