...
dag_id="ReprocessCellxpertExecuteAllOperatorAccounts"
...
with DAG(
    ...
    for res in result:
        if (
            ...
            name_of_task = replace_special_characters(
                f"S1_ Cellxpert {res['name']} {res['username']} {res['operator_id']}"
            )
            task_name = f"ProcessOperator{operator_id}.{name_of_task}"
            task_group_name = f"ProcessOperator{res['operator_id']}"
            stage_1 = replace_special_characters(f"S1_ Cellxpert {res['name']} {res['username']} {res['operator_id']}")
    ...
    trigger_cellxpert_dbt = TriggerDagRunOperator(
        ...
        trigger_rule="all_done",
        conf={'triggered_by': 'reprocess_dag'},  # Mark as triggered by reprocess to prevent loop
    )
    ...
    end >> check_downstream_tasks >> check_dbt_enabled >> trigger_cellxpert_dbt
