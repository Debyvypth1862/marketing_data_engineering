...
gcp_snowflake_s3_bucket = "your-gcp-bucket"
        ...
        for root, _, files in os.walk(temp_dir):
            for file in files:
                if file.endswith(".csv") and file in files_to_upload:
                    ...
                    task_logger.info(
                        f"Uploading {local_file_path} to s3://{gcp_snowflake_s3_bucket}/{s3_key}"
                    )
                    s3_client.upload_file(
                        local_file_path, gcp_snowflake_s3_bucket, s3_key
                    )
