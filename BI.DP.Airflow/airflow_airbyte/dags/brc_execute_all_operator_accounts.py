...
from airbyte.airbyte_jobs import airbyte_api_connections_sync, airbyte_api_connections_sync_v2, handle_airbyte_job_failure
...
spree_brc_operator_id = spree_airbyte_result[3]
...
def check_dag_failure(**context):
    """Check if any tasks failed and fail the DAG if so"""
    ...
    if failed_tasks:
        failed_task_ids = [ti.task_id for ti in failed_tasks]
        raise AirflowFailException(f"DAG failed due to failed tasks: {failed_task_ids}")
...
with DAG(
    ...
    start = PythonOperator(
        task_id="start_task", python_callable=Utils.pause_recovery_dags
    )
    task_groups = []
    for res in result:
        operator_id_tuple = fetch_operator_id_from_ACCOUNT(res[0])
        task_name = f"ProcessOperator{operator_id_tuple[0][0]}.{replace_special_characters(f'S1_ Brc {res[2]} {res[1]} {res[3]}')}"
        task_group_name = f"ProcessOperator{res[3]}"
        stage_1 = replace_special_characters(f"S1_ Brc {res[2]} {res[1]} {res[3]}")
    ...
    spree_sync_task = PythonOperator(
        task_id="airbyte_trigger_spree_brc_sync_to_gbq",
    ...
    update_airbyte_job_status = PythonOperator(
        task_id="update_spree_brc_sync_airbyte_job_status",
        python_callable=update_airbyte_jobs,
        op_kwargs={
            "platform": platform,
            "airbyte_trigger_task_name": "airbyte_trigger_spree_brc_sync_to_gbq",
            "operator_id": spree_brc_operator_id, 
        },
        do_xcom_push=False,
    )
    ...
    # Final task to check if any tasks failed and fail the DAG accordingly
    final_check = PythonOperator(
        task_id="final_failure_check",
        python_callable=check_dag_failure,
        trigger_rule="all_done",
        provide_context=True,
    )
    ...
    end_task_group >> dbt_task >> spree_sync_task >> update_airbyte_job_status >> Jira_create >> end >> final_check
