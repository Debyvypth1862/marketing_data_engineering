...
dag_id="ReprocessSoftswissExecuteAllOperatorAccounts"
...
with DAG(
    ...
    for res in result:
        if (
            ...
            name_of_task = replace_special_characters(
                f"S1_ Softswiss {res['name']} {res['username']} {res['operator_id']}"
            )
            task_name = f"ProcessOperator{operator_id}.{name_of_task}"
            task_group_name = f"ProcessOperator{res['operator_id']}"
            stage_1 = replace_special_characters(f"S1_ Softswiss {res['name']} {res['username']} {res['operator_id']}")
    ...
    end >> check_downstream_tasks >> check_dbt_enabled >> trigger_softswiss_dbt
