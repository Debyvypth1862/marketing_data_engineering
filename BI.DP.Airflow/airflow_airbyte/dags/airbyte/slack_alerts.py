...
def airflow_airbyte_sync_task_slack_alert(response):
    ...
    if flags.get("operator", None) == 1:
        ...
        # Create Jira ticket for failed/incomplete airbyte sync
        if response['job']['status'] != 'succeeded':
            jira_msg = (
                f'Task Name: {task_id}\n'
                f'DAG ID: {dag_id}\n'
                ...
                f'Airbyte Job ID: {job_id}\n'
                f'Airbyte Job Status: {status}\n'
                f'Airbyte Logs: {airbyte_host}workspaces/{workspace_id}/connections/{conn_id}/job-history\n'
                f'Airflow Logs: {airflow_host}{str(airflow_logs_url).split("/")[-1]}\n'
            )
            Utils.create_daily_sync_alert_jira_ticket({
                "title": f"{title} - {dag_id}",
                "desc": jira_msg,
                "task_id": task_id,
                "dag_id": dag_id,
                "dateTime": exec_dt,
...
def airflow_trigger_airbyte_sync_task_error_slack_alert(response):
    ...
    if flags.get("operator", None) == 1:
        ...
        # Create Jira ticket for failed Airbyte sync
        jira_msg = (
            f'Task Name: {task_id}\n'
            f'DAG ID: {dag_id}\n'
            f'Airflow Start DateTime: {exec_dt}\n'
            f'Airbyte Connection ID: {conn_id}\n'
            f'Airbyte Logs: {airbyte_host}workspaces/{workspace_id}/connections/{conn_id}\n'
            f'Airflow Logs: {airflow_host}{str(airflow_logs_url).split("/")[-1]}\n'
        )
        Utils.create_daily_sync_alert_jira_ticket({
            "title": f"{title} - {dag_id}",
...
def task_id_slack_failure_alert(context):
    ...
    if flags.get("jobs", None) == 1:
        ...
        # Create Jira ticket for failed tasks
        jira_msg = (
            f'Task Name: {task_id}\n'
            f'DAG ID: {dag_id}\n'
            f'Task Start Datetime: {exec_dt}\n'
    ...
    if flags.get(dq_flag, None) == 1:
        ...
        try:
            ...
            jira_msg = (
                ...
                f'DAG ID: {dag_id}\n'
                f'Task Start Datetime: {exec_dt}\n'
                f'DQ Logs: {app_base_url}logs?type=dqfail&jobid={job_id}&operatorId={operator_id}'
            )
            Utils.create_daily_sync_alert_jira_ticket({
                "title": f"{title} - {dag_id}",
                "desc": jira_msg,
...
def operator_validation_fail_slack_alert(context, platform, operator_id, error_msg):
    ...
    try:
        ...
        # Create Jira ticket for failed operator validation
        jira_msg = (
            f'Platform: {platform}\n'
            f'Operator ID: {operator_id}\n'
            f'Task ID: {task_id}\n'
            ...
            f'Task Start Datetime: {exec_dt}\n'
            f'Error: {error_msg}\n'
            f'Operator Validation Fail Logs: {app_base_url}logs?type=opValidationFail&operatorId={operator_id}'
        )
        Utils.create_daily_sync_alert_jira_ticket({
            "title": f"{title} - {platform} - {operator_id}",
            "desc": jira_msg,
            "task_id": task_id,
            "dag_id": dag_id,
            "dateTime": exec_dt,
...
def pk_schema_validation_fail_slack_alert(context, operator_id, job_id, validation_type, txn_dts):
    ...
    if flags.get(val_flag, None) == 1:
        ...
        try:
            ...
            # Create Jira ticket for failed validation
            jira_msg = (
                f'Operator ID: {operator_id}\n'
                f'Task ID: {task_id}\n'
                f'DAG ID: {dag_id}\n'
...
def consoliated_dq_fail_slack_alert(context, operator_id, job_id, message):
    ...
    try:
        ...
        # Create Jira ticket for failed DQ
        jira_msg = (
            f'Operator ID: {operator_id}\n'
            f'Task ID: {task_id}\n'
            f'DAG ID: {dag_id}\n'
        ...
        try:
            ...
            logger.warning(f"Failed to send Slack alert for DQ failure: {str(e)}")