...
def remove_duplicates(local_file_path, output_file_path):
    ...
    if '_airbyte_data' in df.columns:
        ...
        task_logger.info("Inside else: where airbyte_data is missing")
...
def read_file_from_s3_date(file_path, platform, retry_count=1):
    max_retries = 5
    try:
        if platform in (constants.BRC, constants.BRT, constants.Google_Analytics, constants.Voluum, constants.Apilayer):
...
def date_values(df, platform):
    ...
    def filter_errors(row):
        if "_airbyte_data" in row and "message" in row["_airbyte_data"]["data"]:
            return row["_airbyte_data"]["data"]["message"] != "Internal server error"
        return True
...
def process_data(df, platform):
    ...
    if platform not in (constants.MyAffiliates, constants.Income_Access, constants.SoftSwiss,
                        constants.Google_Analytics, constants.Voluum, constants.BRC, constants.BRT):
        df = df[df.apply(filter_errors, axis=1)]
...
def process_files_data(filepath, job_id, sync_window, platform,stream):
    ...
    for f_path in files:
        ...
        if env_position != -1:
            ...
            dir_name, file_name = os.path.split(file_path)#changed here
            reprocess_path = os.path.join(dir_name, file_name)#changed here

        task_logger.info(f"process_files_data() processing file file_path: {reprocess_path}")
        df = read_file_from_s3(reprocess_path)#changed here

        current_date_time = datetime.now().strftime("%Y-%m-%d_%I:%M:%S_%p") #changed for reprocress
        insert_position = reprocess_path.rfind("/")
        archive_path = (
                reprocess_path[:insert_position]
                + f"/{current_date_time}"
                + reprocess_path[insert_position:]
        )
        move_object(
            f"{os.getenv('Input_bucket')}",
            f"{os.getenv('Input_bucket_path')}{reprocess_path}",
...
def data_source_item(ti, platform, task_name, operator_id):
    try:
        ...
        # Retrieve the start date for the job from XCom
        task_ids = []
        ...
        #item[0] = id, item[1] = operator_id, item[2] = source_name, item[3] = recovery_airbyte_connection_id, item[4] = path from data source table
        ...
        for job_id, acc in zip(valid_job_ids, acc_list):
            ...
            if sync_window == [""]:
                sync_window = [datetime.now().strftime("%Y-%m-%d")]
