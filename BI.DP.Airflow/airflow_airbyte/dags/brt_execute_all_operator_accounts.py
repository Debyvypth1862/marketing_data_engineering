...
platform = constants.BRT
is_reprocess = False
platform_id = fetch_platform_id_from_platform(platform)
result = fetch_connid_oppid_by_platform(platform_id)
dag_id = "BrtExecuteAllOperatorAccounts"
task_group_and_task_ids = []
batch_size = 1
dbt_working_directory = "/opt/airflow/dags/dbt/brt/brt"
dbt_project = "Brt"
dbt_artifact_bucket = "your-dbt-artifacts-bucket"

brt_platform = constants.BRT
brt_platform_id = fetch_platform_id_from_platform(brt_platform)
brt_airbyte_result = fetch_connid_oppid_by_platform_and_name(brt_platform_id, platform.lower())[0]
brt_airbyte_connection_id = brt_airbyte_result[0]
brt_Brt_operator_id = brt_airbyte_result[3]
...
with DAG(
    ...
    for res in result:
        ...
        task_name = f"ProcessOperator{operator_id_tuple[0][0]}.{replace_special_characters(f'S1_ Brt {res[2]} {res[1]} {res[3]}')}"
        task_group_name = f"ProcessOperator{res[3]}"
        stage_1 = replace_special_characters(f"S1_ Brt {res[2]} {res[1]} {res[3]}")
    ...
    end_task_group >> dbt_task >> Jira_create >> end >> final_check
