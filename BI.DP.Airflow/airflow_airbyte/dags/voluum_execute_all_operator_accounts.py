...
from airbyte.airbyte_jobs import (
    airbyte_api_connections_sync_v2,
    handle_airbyte_job_failure,
    trigger_airbyte_job,
)
...
dag_id="VoluumExecuteAllOperatorAccounts"
task_group_and_task_ids = []
dbt_working_directory = "/opt/airflow/dags/dbt/voluum/voluum"
dbt_project = "Voluum"
...
spree_voluum_operator_id = spree_airbyte_result[3]
...
with DAG(
    ...
    for res in result:
        ...
        task_name = f'ProcessOperator{operator_id_tuple[0][0]}.{replace_special_characters(f"S1_ Voluum {res[2]} {res[1]} {res[3]}")}'
        task_group_name = f"ProcessOperator{res[3]}"
        stage_1 = replace_special_characters(f"S1_ Voluum {res[2]} {res[1]} {res[3]}")
        stage_2 = "S2_UpdateDataExtractionMetadata"
        stage_3 = "S3_TransactionSlicing"
        stage_4 = "S4_DQ"
        no_of_tasks = 4         
        with TaskGroup(
            f"ProcessOperator{res[3]}",
    ...
    spree_sync_task = PythonOperator(
        task_id="airbyte_trigger_spree_voluum_sync_to_gbq",
    ...
    update_airbyte_job_status = PythonOperator(
        task_id="update_spree_voluum_sync_airbyte_job_status",
        python_callable=update_airbyte_jobs,
        op_kwargs={
            "platform": platform,
            "airbyte_trigger_task_name": "airbyte_trigger_spree_voluum_sync_to_gbq",
            "operator_id": spree_voluum_operator_id, 
        },
        do_xcom_push=False,
    )
