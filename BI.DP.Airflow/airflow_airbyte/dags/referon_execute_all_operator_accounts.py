...
from airbyte.airbyte_jobs import airbyte_api_connections_sync
...
dag_id="ReferonExecuteAllOperatorAccounts"
...
with DAG(
    ...
    for res in result:
        ...
        task_name = f'process_operator_{operator_id_tuple[0][0]}.{replace_special_characters(f"S1_ Referon {res[2]}_{res[1]}_{res[3]}")}'
        task_group_name = f"process_operator_{res[3]}"
        stage_1 = replace_special_characters(f"S1_ Referon {res[2]}_{res[1]}_{res[3]}")
    ...
    check_dbt_running = ShortCircuitOperator(
        ...
        op_kwargs={"dag_id_to_check": "ReferonDbtTriggered"},
        provide_context=True,
    )

    # Trigger DBT DAG after main processing completes
    trigger_referon_dbt = TriggerDagRunOperator(
        task_id="trigger_referon_dbt",
        trigger_dag_id="ReferonDbtTriggered",
    ...
    end >> check_downstream_tasks >> check_dbt_enabled >> check_dbt_running >> trigger_referon_dbt
