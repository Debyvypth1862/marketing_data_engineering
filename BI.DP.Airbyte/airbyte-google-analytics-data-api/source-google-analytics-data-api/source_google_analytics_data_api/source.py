...
class GoogleAnalyticsDataApiBaseStream(GoogleAnalyticsDataApiAbstractStream):
    ...
    def stream_slices(
        ...
        if self.config.get("is_recovery", False):
            stream_key = f"{self.name}_recovery_dates"
            recovery_dates_str = self.config.get(stream_key, None)
            if recovery_dates_str:
                r_dates = recovery_dates_str.split(",")
                for d in r_dates:
                    try:
                        dt = datetime.datetime.strptime(d, "%Y-%m-%d")
                        start_date = end_date = utils.date_to_string(dt)
                        ...
                        raise ConfigurationError("recovery dates is not in correct format")
            else:
                return []
            ...
            while start_date <= today:
                ...
                if GoogleAnalyticsQuotaHandler.stop_iter:
                    ...
                    yield {
                        ...
                        "endDate": utils.date_to_string(
                            min(start_date + datetime.timedelta(days=self.config["window_in_days"] - 1), today)),
                    }
