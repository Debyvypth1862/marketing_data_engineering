...
# @dataclass
# class CustomErrorHandler(HttpResponseFilter): 
#     config: Config
#     error_message: Union[InterpolatedString, str] = "Status"
#     error_message_contains: str = "status"
#     def response_contains_error_message(self, response: requests.Response, action : Union[InterpolatedString, str]) -> bool:
...
class CustomRequester(HttpRequester):
    ...
    def max_time(self) -> Union[int, None]:
        ...
        if self.attempt_count < (self.max_retries * 2) + 1:
            return round(20 * (2 ** (self.attempt_count // 2 - 1)))
        # return 300
    ...
    def interpret_response_status(self, response: requests.Response) -> ResponseStatus:
        ...
        try:
            ...
            if status == "Too early":
                ...
                try:
                    error_message = HttpRequester.parse_response_error_message(response)
                except:
                    error_message = ""
                return ResponseStatus(response_action=response_action, retry_in=retry_in, error_message=error_message)
        except:
