...
from typing import Any, Mapping, Optional, List, Iterable, Union, MutableMapping, Dict
import dpath.util
import pendulum
import requests
import xmltodict
...
@dataclass
class CustomAuthenticator(DeclarativeOauth2Authenticator):
    config: Config
    parameters: InitVar[Mapping[str, Any]]
    ...
    oauth = None
    scopes: InterpolatedString | str
    grant_type: InterpolatedString | str
    refresh_access_token_headers: Mapping[str, Any] | None = None
    refresh_access_token_authenticator: HttpAuthenticator | None = None
    _token_expiry_date = pendulum.now().subtract(days=1)
    access_token_name: InterpolatedString | str
    expires_in_name: InterpolatedString | str
...
@dataclass
class CustomRecordExtractor_for_xml(RecordExtractor):
    parameters: InitVar[Mapping[str, Any]]
    config: Config
    ...
    def extract_records(self, response: requests.Response) -> list[dict[str | Any, str | Any]]:
