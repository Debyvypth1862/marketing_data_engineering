...
class CustomDateTimeBasedCursor(DatetimeBasedCursor):
    ...
    def stream_slices(self) -> Iterable[StreamSlice]:
        logger.info(f"APISIX_ENABLED --> {os.environ['APISIX_ENABLED']}")
        logger.info(f"APISIX_ENDPOINT --> {os.environ['APISIX_ENDPOINT']}")
    ...
    def extract_records(self, response: requests.Response) -> List[Record]:
        ...
        if 'hasData' in response_body:
            if response_body['hasData']:
                ...
                # logger.info(f'length of data_values ---> {len(data_values)}')
                final_response=[]
                actual_response=[]
