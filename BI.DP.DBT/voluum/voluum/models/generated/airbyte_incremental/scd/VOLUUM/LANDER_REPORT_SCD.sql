{{ config(
    ...
    post_hook = ["
                    ...
                    "drop view {{ ref('LANDER_REPORT_STG') }}"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('LANDER_REPORT_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('LANDER_REPORT_STG')  }}
    -- CONVERSIONS from {{ source('VOLUUM', '_AIRBYTE_RAW_LANDER_REPORT') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('LANDER_REPORT_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('LANDER_REPORT_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('LANDER_REPORT_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
    from {{ ref('LANDER_REPORT_STG')  }}
    -- LANDER_REPORT from {{ source('VOLUUM', '_AIRBYTE_RAW_LANDER_REPORT') }}
),
{% endif %}
scd_data as (
    ...
    DATE as _AIRBYTE_START_AT,
      lag(DATE) over (
        partition by DATE, LANDER_ID, LANDER_WORKSPACE_ID
        order by
            DATE is null asc,
            DATE desc,
