    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('CAMPAIGN_REPORT')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('CAMPAIGN_REPORT')) }}
                    ...
                    "drop view {{ ref('CAMPAIGN_REPORT_STG') }}"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('CAMPAIGN_REPORT_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('CAMPAIGN_REPORT_STG')  }}
    -- CAMPAIGN_REPORT from {{ source('VOLUUM', '_AIRBYTE_RAW_CAMPAIGN_REPORT') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('CAMPAIGN_REPORT_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('CAMPAIGN_REPORT_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('CAMPAIGN_REPORT_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      {{ dbt_utils.surrogate_key([
        ...
        OFFER_PAYOUTIN_ORIGINAL_CURRENCY,
        OFFER_URL,
        OFFER_WORKSPACE_ID,
    ...
    DATE as _AIRBYTE_START_AT,
      lag(DATE) over (
        partition by DATE, CREATED, OFFER_ID, OFFER_WORKSPACE_ID
        order by
            DATE is null asc,
            DATE desc,
      ...
      _AIRBYTE_EMITTED_AT,
      S3_PATH,
      _AIRBYTE_CAMPAIGN_REPORT_HASHID
    from input_data
),
dedup_data as (
    select
