{{ config(
    ...
    post_hook = ["
                    ...
                    "drop view {{ ref('OFFER_REPORT_STG') }}"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('OFFER_REPORT_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('OFFER_REPORT_STG')  }}
    -- CONVERSIONS from {{ source('VOLUUM', '_AIRBYTE_RAW_OFFER_REPORT') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('OFFER_REPORT_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('OFFER_REPORT_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('OFFER_REPORT_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    DATE as _AIRBYTE_START_AT,
      lag(DATE) over (
        partition by DATE, OFFER_ID, OFFER_WORKSPACE_ID
        order by
            DATE is null asc,
            DATE desc,
