{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='AFFILIATE_NETWORK_REPORT'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('AFFILIATE_NETWORK_REPORT')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('AFFILIATE_NETWORK_REPORT')) }}
                    ...
                    "drop view {{ ref('AFFILIATE_NETWORK_REPORT_STG') }}"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('AFFILIATE_NETWORK_REPORT_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('AFFILIATE_NETWORK_REPORT_STG')  }}
    -- AFFILIATE_NETWORK_REPORT from {{ source('VOLUUM', '_AIRBYTE_RAW_AFFILIATE_NETWORK_REPORT') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('AFFILIATE_NETWORK_REPORT_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('AFFILIATE_NETWORK_REPORT_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('AFFILIATE_NETWORK_REPORT_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    DATE as _AIRBYTE_START_AT,
      lag(DATE) over (
        partition by DATE, CREATED, AFFILIATE_NETWORK_ID, AFFILIATE_NETWORK_WORKSPACE_ID
        order by
            DATE is null asc,
            DATE desc,
