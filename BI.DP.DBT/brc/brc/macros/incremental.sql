...
{%- macro emitted_at_clause(col_emitted_at, tablename) -%}
{% if not is_incremental() %}
    {% if get_max_emitted_at(col_emitted_at, tablename) %}
and cast({{ col_emitted_at }} as {{ type_timestamp_with_timezone() }}) >=
    cast('{{ get_max_emitted_at(col_emitted_at, tablename) }}' as {{ type_timestamp_with_timezone() }})
    {% endif %}
{% endif %}
{%- endmacro -%}
...
and ((select max(cast({{ col_emitted_at }} as {{ type_timestamp_with_timezone() }})) from {{ tablename }}) is null
  or cast({{ col_emitted_at }} as {{ type_timestamp_with_timezone() }}) >
     (select dateadd(hour,24,max(cast({{ col_emitted_at }} as {{ type_timestamp_with_timezone() }}))) from {{ tablename }}))
{% endif %}
{%- endmacro -%}
...
{% if execute and is_incremental() %}
 {% if env_var('INCREMENTAL_CURSOR', 'UNSET') == 'UNSET' %}
     {% set query %}
        select dateadd(day, -2, max(cast({{ col_emitted_at }} as {{ type_timestamp_with_timezone() }}))) from {{ tablename }}
     {% endset %}
...
{% macro get_max_emitted_at(col_emitted_at, tablename) %}
{% if execute %}
