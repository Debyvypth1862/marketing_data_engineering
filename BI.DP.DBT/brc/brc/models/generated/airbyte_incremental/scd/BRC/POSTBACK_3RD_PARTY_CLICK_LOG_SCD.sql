{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='POSTBACK_3RD_PARTY_CLICK_LOG'
                        )
                    %}
                    {#
                    ...
                    FROM (SELECT S3_PATH, count(*) as RECORD_COUNT
                     ...
                     "drop view {{ this.database }}.BRC.POSTBACK_3RD_PARTY_CLICK_LOG_STG"],
                    
    tags = [ "top-level" ]
) }}
-- depends_on: ref('POSTBACK_3RD_PARTY_CLICK_LOG_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('POSTBACK_3RD_PARTY_CLICK_LOG_STG')  }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('POSTBACK_3RD_PARTY_CLICK_LOG_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('POSTBACK_3RD_PARTY_CLICK_LOG_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('POSTBACK_3RD_PARTY_CLICK_LOG_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
	...
	POST_SUBID_4,
	POST_SUBID_5,
    POST_CLICK_TIMESTAMP as _AIRBYTE_START_AT,
      lag(POST_CLICK_TIMESTAMP) over (
        partition by POST_CLICK_TIMESTAMP, POST_ID, POST_CLICKID
        order by
            POST_CLICK_TIMESTAMP is null asc,
            POST_CLICK_TIMESTAMP desc,
