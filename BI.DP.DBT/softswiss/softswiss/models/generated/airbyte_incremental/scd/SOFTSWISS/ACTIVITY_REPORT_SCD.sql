    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('ACTIVITY_REPORT')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('ACTIVITY_REPORT')) }}
                    ...
                    FROM (SELECT S3_PATH, count(*) as RECORD_COUNT
                     ...
                     "drop view {{ this.database }}.SOFTSWISS.ACTIVITY_REPORT_STG"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('ACTIVITY_REPORT_STG')
with
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('ACTIVITY_REPORT_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('ACTIVITY_REPORT_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('ACTIVITY_REPORT_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      lag(DATE) over (
        partition by DATE, DYNAMIC_TAG_CLICKID, TRACKER_LOGIN_ID,BRAND_ID,CAMPAIGN_ID,CURRENCY
        order by
            DATE is null asc,
            DATE desc,
