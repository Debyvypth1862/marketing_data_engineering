{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='FTD_REGISTRATION_REPORT'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('FTD_REGISTRATION_REPORT')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('FTD_REGISTRATION_REPORT')) }}
                    ...
                    "drop view {{ ref('FTD_REGISTRATION_REPORT_STG') }}"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('FTD_REGISTRATION_REPORT_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('FTD_REGISTRATION_REPORT_STG')  }}
    -- FTD_REGISTRATION_REPORT from {{ source('SWEEP', '_AIRBYTE_RAW_FTD_REGISTRATION_REPORT_STREAM') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('FTD_REGISTRATION_REPORT_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('FTD_REGISTRATION_REPORT_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('FTD_REGISTRATION_REPORT_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
