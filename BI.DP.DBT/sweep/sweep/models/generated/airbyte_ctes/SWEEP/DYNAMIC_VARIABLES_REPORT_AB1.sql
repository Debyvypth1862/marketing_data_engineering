{{ config(
    ...
    schema = "SWEEP",
    pre_hook = "ALTER EXTERNAL TABLE STG.SWEEP._AIRBYTE_RAW_DYNAMIC_VARIABLES_REPORT_STREAM REFRESH;",
    tags = [ "top-level-intermediate" ],
) }}
-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: {{ source('SWEEP', '_AIRBYTE_RAW_DYNAMIC_VARIABLES_REPORT_STREAM') }}
WITH s3_status AS (
...
from {{ source('SWEEP', '_AIRBYTE_RAW_DYNAMIC_VARIABLES_REPORT_STREAM') }} as table_alias
JOIN s3_status s3
