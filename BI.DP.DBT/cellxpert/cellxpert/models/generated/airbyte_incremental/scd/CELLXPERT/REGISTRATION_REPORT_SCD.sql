{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='REGISTRATION_REPORT'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('REGISTRATION_REPORT')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('REGISTRATION_REPORT')) }}
                   ...
                   "UPDATE {{ env_var('snowflake_db_dbt') }}.{{ env_var('snowflake_schema_dbt') }}.{{ env_var('snowflake_table_dbt') }}
                    ...
                    "drop view {{ this.database }}.CELLXPERT.REGISTRATION_REPORT_STG"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('REGISTRATION_REPORT_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('REGISTRATION_REPORT_STG')  }}
    -- REGISTRATION_REPORT from {{ source('CELLXPERT', '_AIRBYTE_RAW_REGISTRATION_REPORT_STREAM') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('REGISTRATION_REPORT_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('REGISTRATION_REPORT_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('REGISTRATION_REPORT_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
