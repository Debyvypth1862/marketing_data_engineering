{{ config(
    ...
    post_hook = ["
                    ...
                    "drop view {{ this.database }}.CELLXPERT.DYNAMIC_VARIABLES_REPORT_STG"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- DYNAMIC_VARIABLES_REPORT from {{ source('CELLXPERT', '_AIRBYTE_RAW_DYNAMIC_VARIABLES_REPORT_STREAM') }}
    where 1 = 1
...
scd_data as (
    ...
    select
      ...
      lag(DATE) over (
        partition by DATE, USERID, TRACKER_LOGIN_ID
        order by
            DATE is null asc,
            DATE desc,
