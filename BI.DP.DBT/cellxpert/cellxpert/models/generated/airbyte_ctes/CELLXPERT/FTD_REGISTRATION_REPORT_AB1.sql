{{ config(
    ...
    schema = "_AIRBYTE_CELLXPERT",
    tags = [ "top-level-intermediate" ]
) }}
-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: {{ source('CELLXPERT', '_AIRBYTE_RAW_FTD_REGISTRATION_REPORT_STREAM') }}
WITH s3_status AS (
...
from {{ source('CELLXPERT', '_AIRBYTE_RAW_FTD_REGISTRATION_REPORT_STREAM') }} as table_alias
JOIN s3_status s3
