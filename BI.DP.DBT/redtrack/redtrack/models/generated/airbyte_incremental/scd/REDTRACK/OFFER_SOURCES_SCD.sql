    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('OFFER_SOURCES')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('OFFER_SOURCES')) }}
                    ...
                    ",
                    "drop view {{ ref('OFFER_SOURCES_STG') }}"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('OFFER_SOURCES_STG')
with
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('OFFER_SOURCES_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('OFFER_SOURCES_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('OFFER_SOURCES_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
