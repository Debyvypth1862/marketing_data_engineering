    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('CLICKS')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('CLICKS')) }}
                    ...
                    ",
                    "drop view {{ ref('CLICKS_STG') }}"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('CLICKS_STG')
with
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('CLICKS_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('CLICKS_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('CLICKS_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
        ...
        USER_AGENT,
        USER_ID,
        TRACK_TIME as _AIRBYTE_START_AT,
        lag(TRACK_TIME) over (
            partition by TRACK_TIME, ID, LANDING_ID
            order by
                TRACK_TIME is null asc,
                TRACK_TIME desc,
