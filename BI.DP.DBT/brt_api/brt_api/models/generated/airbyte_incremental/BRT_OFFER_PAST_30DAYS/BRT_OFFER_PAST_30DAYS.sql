...
-- Final table for BRT_OFFER_PAST_30DAYS with created_at/modified_at
-- Reads from SCD table and adds user-friendly timestamp columns
-- depends_on: {{ ref('BRT_OFFER_PAST_30DAYS_SCD') }}

WITH scd_active AS (
    SELECT
        *
    FROM {{ ref('BRT_OFFER_PAST_30DAYS_SCD') }}
...
first_version AS (
    SELECT
        ...
        MIN(_AIRBYTE_BRT_OFFER_PAST_30DAYS_HASHID) as first_hash
    FROM {{ ref('BRT_OFFER_PAST_30DAYS_SCD') }}
...
SELECT
    ...
    scd._AIRBYTE_BRT_OFFER_PAST_30DAYS_HASHID,
    ...
    CASE
        WHEN scd._AIRBYTE_BRT_OFFER_PAST_30DAYS_HASHID = fv.first_hash THEN NULL
