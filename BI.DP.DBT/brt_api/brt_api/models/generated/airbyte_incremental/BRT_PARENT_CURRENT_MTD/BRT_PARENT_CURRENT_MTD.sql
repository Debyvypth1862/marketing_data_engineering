...
-- Final table for BRT_PARENT_CURRENT_MTD with created_at/modified_at
-- Reads from SCD table and adds user-friendly timestamp columns
-- depends_on: {{ ref('BRT_PARENT_CURRENT_MTD_SCD') }}

WITH scd_active AS (
    SELECT
        *
    FROM {{ ref('BRT_PARENT_CURRENT_MTD_SCD') }}
...
first_version AS (
    SELECT
        ...
        MIN(_AIRBYTE_BRT_PARENT_CURRENT_MTD_HASHID) as first_hash
    FROM {{ ref('BRT_PARENT_CURRENT_MTD_SCD') }}
...
SELECT
    ...
    scd._AIRBYTE_BRT_PARENT_CURRENT_MTD_HASHID,
    ...
    CASE
        WHEN scd._AIRBYTE_BRT_PARENT_CURRENT_MTD_HASHID = fv.first_hash THEN NULL
