{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema='BRT',
                            identifier='BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH'
                        )
                    %}
                    {%
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', env_var('EXP_DATABASE', 'EXP') ~ '.BRT.BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH') }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', env_var('EXP_DATABASE', 'EXP') ~ '.BRT.BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH') }}
                    ...
                    {% endif %}
                    ",
                    "drop view {{ env_var('INTM_DATABASE', 'INTM') }}.BRT.BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH_STG"],
    tags = [ "top-level" ]
) }}
-- SCD Type 2 model for BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH
-- depends_on: {{ ref('BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH_STG') }}
WITH
{% if is_incremental() %}
new_data AS (
    ...
    SELECT
        *
    FROM {{ ref('BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH_STG') }}
    WHERE 1 = 1
...
previous_active_scd_data AS (
    ...
    SELECT
        {{ star_intersect(ref('BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data AS (
    SELECT {{ dbt_utils.star(ref('BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH_STG')) }} FROM new_data
    UNION ALL
    SELECT {{ dbt_utils.star(ref('BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH_STG')) }} FROM previous_active_scd_data
),
{% else %}
input_data AS (
    SELECT *
...
scd_data AS (
    ...
    SELECT
        ...
        _AIRBYTE_AB_ID,
        _AIRBYTE_EMITTED_AT,
        _AIRBYTE_BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH_HASHID
    FROM input_data
),
dedup_data AS (
