    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', env_var('EXP_DATABASE', 'EXP') ~ '.PUBLIC.BRT_PARENT_ALL') }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', env_var('EXP_DATABASE', 'EXP') ~ '.PUBLIC.BRT_PARENT_ALL') }}
                    ...
                    {% endif %}
                    ",
                    "drop view {{ env_var('INTM_DATABASE', 'INTM') }}.BRT.BRT_PARENT_ALL_STG"],
    tags = [ "top-level" ]
) }}
-- SCD Type 2 model for BRT_PARENT_ALL
-- depends_on: {{ ref('BRT_PARENT_ALL_STG') }}
WITH
{% if is_incremental() %}
new_data AS (
...
previous_active_scd_data AS (
    ...
    SELECT
        {{ star_intersect(ref('BRT_PARENT_ALL_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data AS (
    SELECT {{ dbt_utils.star(ref('BRT_PARENT_ALL_STG')) }} FROM new_data
    UNION ALL
    SELECT {{ dbt_utils.star(ref('BRT_PARENT_ALL_STG')) }} FROM previous_active_scd_data
),
{% else %}
input_data AS (
    SELECT *
...
scd_data AS (
    ...
    SELECT
        ...
        LAG(_AIRBYTE_EMITTED_AT) OVER (
            PARTITION BY CLICK_DATE, PARENT_ID
            ORDER BY
        ...
        _AIRBYTE_AB_ID,
        _AIRBYTE_EMITTED_AT,
        _AIRBYTE_BRT_PARENT_ALL_HASHID
    FROM input_data
),
dedup_data AS (
    SELECT
