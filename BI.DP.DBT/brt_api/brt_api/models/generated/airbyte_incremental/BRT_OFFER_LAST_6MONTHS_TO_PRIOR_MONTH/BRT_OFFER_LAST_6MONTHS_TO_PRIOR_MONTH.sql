...
-- Final table for BRT_OFFER_LAST_6MONTHS_TO_PRIOR_MONTH with created_at/modified_at
-- Reads from SCD table and adds user-friendly timestamp columns
-- depends_on: {{ ref('BRT_OFFER_LAST_6MONTHS_TO_PRIOR_MONTH_SCD') }}

WITH scd_active AS (
    SELECT
        *
    FROM {{ ref('BRT_OFFER_LAST_6MONTHS_TO_PRIOR_MONTH_SCD') }}
...
first_version AS (
    SELECT
        ...
        MIN(_AIRBYTE_BRT_OFFER_LAST_6MONTHS_TO_PRIOR_MONTH_HASHID) as first_hash
    FROM {{ ref('BRT_OFFER_LAST_6MONTHS_TO_PRIOR_MONTH_SCD') }}
...
SELECT
    ...
    scd._AIRBYTE_BRT_OFFER_LAST_6MONTHS_TO_PRIOR_MONTH_HASHID,
    ...
    CASE
        WHEN scd._AIRBYTE_BRT_OFFER_LAST_6MONTHS_TO_PRIOR_MONTH_HASHID = fv.first_hash THEN NULL
