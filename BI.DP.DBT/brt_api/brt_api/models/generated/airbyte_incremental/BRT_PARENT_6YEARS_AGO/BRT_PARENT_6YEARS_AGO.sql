...
-- Final table for BRT_PARENT_6YEARS_AGO with created_at/modified_at
-- Reads from SCD table and adds user-friendly timestamp columns
-- depends_on: {{ ref('BRT_PARENT_6YEARS_AGO_SCD') }}

WITH scd_active AS (
    SELECT
        *
    FROM {{ ref('BRT_PARENT_6YEARS_AGO_SCD') }}
...
first_version AS (
    SELECT
        ...
        MIN(_AIRBYTE_BRT_PARENT_6YEARS_AGO_HASHID) as first_hash
    FROM {{ ref('BRT_PARENT_6YEARS_AGO_SCD') }}
...
SELECT
    ...
    scd._AIRBYTE_BRT_PARENT_6YEARS_AGO_HASHID,
    ...
    CASE
        WHEN scd._AIRBYTE_BRT_PARENT_6YEARS_AGO_HASHID = fv.first_hash THEN NULL
