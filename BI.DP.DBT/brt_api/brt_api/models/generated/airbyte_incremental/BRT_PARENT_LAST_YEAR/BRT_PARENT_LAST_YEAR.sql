...
-- Final table for BRT_PARENT_LAST_YEAR with created_at/modified_at
-- Reads from SCD table and adds user-friendly timestamp columns
-- depends_on: {{ ref('BRT_PARENT_LAST_YEAR_SCD') }}

WITH scd_active AS (
    SELECT
        *
    FROM {{ ref('BRT_PARENT_LAST_YEAR_SCD') }}
...
first_version AS (
    SELECT
        ...
        MIN(_AIRBYTE_BRT_PARENT_LAST_YEAR_HASHID) as first_hash
    FROM {{ ref('BRT_PARENT_LAST_YEAR_SCD') }}
...
SELECT
    ...
    scd._AIRBYTE_BRT_PARENT_LAST_YEAR_HASHID,
    ...
    CASE
        WHEN scd._AIRBYTE_BRT_PARENT_LAST_YEAR_HASHID = fv.first_hash THEN NULL
