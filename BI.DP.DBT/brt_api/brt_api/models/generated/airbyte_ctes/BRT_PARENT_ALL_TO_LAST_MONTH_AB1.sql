...
-- Base transformation for BRT_PARENT_ALL_TO_LAST_MONTH
-- Converts V_BRT_PARENT_ALL_TO_LAST_MONTH.sql to dbt model with source() references
...
(
  SELECT 
      'All to Prior Month' as DATE_RANGE,
      off.PARENT_ID,
      off.ID as OFFER_ID,
      Case 
  ...
  WHERE 
      substr(brc.STATS_END_DATE,1,4)||'-'||substr(brc.STATS_END_DATE,6,2) <= SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),6,2)
  GROUP BY ALL
),
...
(
  ...
  WHERE 
      ...
      AND substr(brc.STATS_END_DATE,1,4)||'-'||substr(brc.STATS_END_DATE,6,2) <= SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),6,2)
  GROUP BY ALL
),
...
(
  ...
  FROM EXP.PUBLIC."FACT_OFFER" fct
...
(
  ...
  WHERE COALESCE(reg.Event_Month, ftd.Event_Month, rev.Event_Month) <= SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),6,2)
  GROUP BY ALL
),
