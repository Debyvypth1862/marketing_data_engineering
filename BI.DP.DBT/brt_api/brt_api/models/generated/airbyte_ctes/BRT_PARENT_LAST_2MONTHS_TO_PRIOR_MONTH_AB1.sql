...
-- Base transformation for BRT_PARENT_LAST_2MONTHS_TO_PRIOR_MONTH
-- Converts V_BRT_PARENT_LAST_2MONTHS_TO_PIOR_MONTH.sql to dbt model with source() references
...
(
  SELECT 
      'Last 2 Months to Prior Month' as DATE_RANGE,
      off.PARENT_ID,
      off.ID as OFFER_ID,
      Case 
  ...
  WHERE 
      substr(brc.STATS_END_DATE,1,4)||'-'||substr(brc.STATS_END_DATE,6,2) BETWEEN SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -2),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -2),6,2) AND 
...
(
  ...
  WHERE 
      ...
      AND substr(brc.STATS_END_DATE,1,4)||'-'||substr(brc.STATS_END_DATE,6,2) BETWEEN SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -2),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -2),6,2) AND 
  SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),6,2)
  GROUP BY ALL
),
...
(
  ...
  WHERE COALESCE(reg.Event_Month, ftd.Event_Month, rev.Event_Month) BETWEEN SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -2),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -2),6,2) AND 
  SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),6,2)
  GROUP BY ALL
),
