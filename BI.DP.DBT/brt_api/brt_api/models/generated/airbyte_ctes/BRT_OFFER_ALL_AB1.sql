...
-- Base transformation for BRT_OFFER_ALL
-- Converts V_BRT_OFFER_ALL.sql to dbt model with source() references
...
(
    SELECT
        STATS_END_DATE as Click_Date,
        off.ID as OFFER_ID,
        CASE
            WHEN a.CAMP_DEPOSIT_BASELINE = '' THEN 0
            WHEN a.CAMP_DEPOSIT_BASELINE = NULL THEN 0
            ELSE TO_NUMBER(a.CAMP_DEPOSIT_BASELINE)
        END as Campaign_Baseline,
        CASE
            WHEN a.CAMP_WAGER_BASELINE = '' THEN 0
            WHEN a.CAMP_WAGER_BASELINE = NULL THEN 0
            ELSE TO_NUMBER(a.CAMP_WAGER_BASELINE)
    ...
    JOIN OFFER off
        ON brc.Tracking_ID = off.Tracking_ID
...
(
    SELECT
        brc.STATS_END_DATE as Click_Date,
        off.ID as OFFER_ID,
        off.tracking_id,
        brc.campaign_name,
        CASE
    ...
    LEFT OUTER JOIN {{ source('BRC', 'ADVERTISERS') }} adv
        ON trk.TLOG_FK_ADVERTISER = adv.ADVE_ID
    WHERE
        brc.TRACKER_ID NOT IN (SELECT Tracker_Login_Id FROM OPERATOR_TRACKER)
    GROUP BY ALL
),
...
(
    SELECT
        COALESCE(clks.Click_Date, conv.Click_Date) as Click_Date,
    ...
    FULL OUTER JOIN BRC_CONVERSIONS conv
        ON clks.Click_Date = conv.Click_Date AND clks.OFFER_ID = conv.OFFER_ID
    WHERE COALESCE(clks.OFFER_ID, conv.OFFER_ID) IS NOT NULL
    GROUP BY ALL
),
...
(
    SELECT
        COALESCE(reg.Click_Date, ftd.Click_Date, rev.Click_Date) as Click_Date,
    ...
    FROM OPERATOR_REGISTRATION reg
...
(
    SELECT
        COALESCE(brc.Click_Date, ops.Click_Date) as Click_Date,
        ...
        IFNULL(CASE
            ...
            ELSE SUM(brc.RevShare_Revenue)
        END,0) AS RevShare_Revenue
    FROM BRC_CONSOLIDATION brc
    FULL OUTER JOIN OPERATOR_CONSOLIDATION ops
        ON brc.Click_Date = ops.Click_Date AND brc.Offer_ID = ops.Offer_ID
    GROUP BY ALL
)

SELECT
    rev.Click_Date as CLICK_DATE,
    rev.Offer_ID as OFFER_ID,
    rev.Campaign_Baseline AS BASELINE_DEPOSIT,
    rev.Campaign_Wager_Baseline AS BASELINE_WAGER,
    rev.CPA_IN,
    rev.CPA_OUT,
    rev.CPA_Diff as CPA_DIFF,
    rev.CPL_IN,
    rev.CPL_OUT,
    rev.CPL_Diff as CPL_DIFF,
    rev.RevShare_In as REVSHARE_IN,
    rev.RevShare_Out as REVSHARE_OUT,
    rev.RevShare_Diff as REVSHARE_DIFF,
    rev.From_BRC as FROM_BRC,
    IFNULL(SUM(rev.CLICK_TOTAL),0) AS CLICK_CNT,
    IFNULL(SUM(rev.FTD_CNT),0) AS FTD_CNT,
    ...
    {{ dbt_utils.surrogate_key(['rev.Click_Date', 'rev.Offer_ID']) }} as _AIRBYTE_AB_ID
