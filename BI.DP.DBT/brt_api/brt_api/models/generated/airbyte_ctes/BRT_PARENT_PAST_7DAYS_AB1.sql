...
-- Base transformation for BRT_PARENT_PAST_7DAYS
-- Converts V_BRT_PARENT_PAST_7DAYS.sql to dbt model with source() references
...
(
  ...
  WHERE 
      brc.STATS_END_DATE BETWEEN CURRENT_DATE - 7 AND CURRENT_DATE - 1
  GROUP BY ALL
),
...
(
  ...
  WHERE 
      ...
      AND brc.STATS_END_DATE BETWEEN CURRENT_DATE - 7 AND CURRENT_DATE - 1
  GROUP BY ALL
),
...
(
  ...
  WHERE COALESCE(reg.Click_Date, ftd.Click_Date, rev.Click_Date) BETWEEN CURRENT_DATE - 7 AND CURRENT_DATE - 1
  GROUP BY ALL
),
