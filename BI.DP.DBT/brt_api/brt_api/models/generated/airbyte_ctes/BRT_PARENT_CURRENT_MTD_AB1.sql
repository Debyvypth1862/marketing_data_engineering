...
-- Base transformation for BRT_PARENT_CURRENT_MTD
-- Converts V_BRT_PARENT_CURRENT_MTD.sql to dbt model with source() references
...
(
  ...
  WHERE 
      brc.STATS_END_DATE BETWEEN DATE_TRUNC(month, CURRENT_DATE() - 1) and CURRENT_DATE - 1
  GROUP BY ALL
),
...
(
  ...
  WHERE 
      ...
      AND brc.STATS_END_DATE BETWEEN DATE_TRUNC(month, CURRENT_DATE() - 1) and CURRENT_DATE - 1
  GROUP BY ALL
),
...
(
  ...
  WHERE COALESCE(reg.Click_Date, ftd.Click_Date, rev.Click_Date) BETWEEN DATE_TRUNC(month, CURRENT_DATE() - 1) and CURRENT_DATE - 1
  GROUP BY ALL
),
