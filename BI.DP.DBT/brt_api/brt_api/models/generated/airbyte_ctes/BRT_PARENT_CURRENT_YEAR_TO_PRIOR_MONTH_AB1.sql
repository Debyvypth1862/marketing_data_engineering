...
-- Base transformation for BRT_PARENT_CURRENT_YEAR_TO_PRIOR_MONTH
-- Converts V_BRT_PARENT_CURRENT_YEAR_TO_PIOR_MONTH.sql to dbt model with source() references
...
(
  SELECT 
      'Current Year to Prior Month' as DATE_RANGE,
      off.PARENT_ID,
      off.ID as OFFER_ID,
      Case 
  ...
  WHERE 
      substr(brc.STATS_END_DATE,1,4)||'-'||substr(brc.STATS_END_DATE,6,2) BETWEEN extract('year',current_date - 1)||'-'||'01' AND 
...
(
  ...
  WHERE 
      ...
      AND substr(brc.STATS_END_DATE,1,4)||'-'||substr(brc.STATS_END_DATE,6,2) BETWEEN extract('year',current_date - 1)||'-'||'01' AND 
  SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),6,2)
  GROUP BY ALL
),
...
(
  ...
  WHERE COALESCE(reg.Event_Month, ftd.Event_Month, rev.Event_Month) BETWEEN extract('year',current_date - 1)||'-'||'01' AND 
  SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),1,4)||'-'||SUBSTR(ADD_MONTHS(CURRENT_DATE -1, -1),6,2)
  GROUP BY ALL
),
