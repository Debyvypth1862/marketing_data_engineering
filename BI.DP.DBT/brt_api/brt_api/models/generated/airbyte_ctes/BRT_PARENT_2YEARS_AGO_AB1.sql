...
-- Base transformation for BRT_PARENT_2YEARS_AGO
-- Converts V_BRT_PARENT_2YEARS_AGO.sql to dbt model with source() references
...
(
  ...
  WHERE 
      substr(brc.STATS_END_DATE,1,4) BETWEEN extract('year',current_date - 1) - 2 AND extract('year',current_date - 1) - 1 
  GROUP BY ALL
),
...
(
  ...
  WHERE 
      ...
      AND substr(brc.STATS_END_DATE,1,4) BETWEEN extract('year',current_date - 1) - 2 AND extract('year',current_date - 1) - 1 
  GROUP BY ALL
),
...
(
  ...
  WHERE SUBSTR(COALESCE(reg.Event_Month, ftd.Event_Month, rev.Event_Month),1,4) BETWEEN extract('year',current_date - 1) - 2 AND extract('year',current_date - 1) - 1
  GROUP BY ALL
),
