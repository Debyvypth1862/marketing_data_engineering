...
-- Base transformation for BRT_PARENT_LAST_YEAR
-- Converts V_BRT_PARENT_LAST_YEAR.sql to dbt model with source() references
...
(
  ...
  WHERE 
      substr(brc.STATS_END_DATE,1,4) = extract('year',current_date - 1) - 1
  GROUP BY ALL
),
...
(
  ...
  WHERE 
      ...
      AND substr(brc.STATS_END_DATE,1,4) = extract('year',current_date - 1) - 1
  GROUP BY ALL
),
...
(
  ...
  WHERE SUBSTR(COALESCE(reg.Event_Month, ftd.Event_Month, rev.Event_Month),1,4) = extract('year',current_date - 1) - 1
  GROUP BY ALL
),
