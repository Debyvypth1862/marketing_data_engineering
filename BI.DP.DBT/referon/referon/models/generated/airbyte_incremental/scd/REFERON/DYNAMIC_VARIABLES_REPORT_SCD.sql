{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='DYNAMIC_VARIABLES_REPORT'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('DYNAMIC_VARIABLES_REPORT')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('DYNAMIC_VARIABLES_REPORT')) }}
                    ...
                    "drop view {{ this.database }}.REFERON.DYNAMIC_VARIABLES_REPORT_STG"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('DYNAMIC_VARIABLES_REPORT_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('DYNAMIC_VARIABLES_REPORT_STG')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('DYNAMIC_VARIABLES_REPORT_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('DYNAMIC_VARIABLES_REPORT_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('DYNAMIC_VARIABLES_REPORT_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    CLICK_COUNT,
      ...
      lag(DATE) over (
        partition by CLICKID, CUSTOMER_ID, PROGRAM_ID, PROGRAM_NAME, BRAND_ID, BRAND_NAME, MARKETING_SOURCE_ID,MARKETING_SOURCE, MEDIA_ITEM_ID,MEDIA_ITEM_NAME, PRODUCT_ID, PRODUCT_NAME, MEDIA_CAMPAIGN_ID,MEDIA_CAMPAIGN_NAME, TRACKER_LOGIN_ID, DATE
        order by
            DATE is null asc,
            DATE desc,
