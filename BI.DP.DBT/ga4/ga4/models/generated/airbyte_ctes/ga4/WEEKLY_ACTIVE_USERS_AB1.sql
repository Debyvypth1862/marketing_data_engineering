...
-- depends_on: {{ source('GOOGLE_ANALYTICS', '_AIRBYTE_RAW_WEEKLY_ACTIVE_USERS') }}
WITH s3_status AS (
    SELECT PATH, IS_PROCESSED, PICKED_FOR_REPROCESS
    FROM {{ source('PUBLIC', 'S3_FILES_STATS') }} 
)
select
    {{ json_extract_scalar('_airbyte_data', ['active7DayUsers'], ['active7DayUsers']) }} as ACTIVE_7DAY_USERS,
...
from {{ source('GOOGLE_ANALYTICS', '_AIRBYTE_RAW_WEEKLY_ACTIVE_USERS') }} as table_alias
JOIN s3_status s3
