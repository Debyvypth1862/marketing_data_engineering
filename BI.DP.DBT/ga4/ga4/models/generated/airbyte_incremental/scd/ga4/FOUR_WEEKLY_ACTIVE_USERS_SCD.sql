{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='FOUR_WEEKLY_ACTIVE_USERS'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('FOUR_WEEKLY_ACTIVE_USERS')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('FOUR_WEEKLY_ACTIVE_USERS')) }}
                    ...
                    "drop view {{ this.database }}.GOOGLE_ANALYTICS.FOUR_WEEKLY_ACTIVE_USERS_STG"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('FOUR_WEEKLY_ACTIVE_USERS_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('FOUR_WEEKLY_ACTIVE_USERS_STG')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('FOUR_WEEKLY_ACTIVE_USERS_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('FOUR_WEEKLY_ACTIVE_USERS_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('FOUR_WEEKLY_ACTIVE_USERS_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      lag(DATE) over (
        partition by DATE, UUID, PROPERTY_ID 
        order by
            DATE is null asc,
            DATE desc,
