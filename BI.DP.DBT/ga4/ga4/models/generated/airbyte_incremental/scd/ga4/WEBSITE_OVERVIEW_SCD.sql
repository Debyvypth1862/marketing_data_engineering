    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('WEBSITE_OVERVIEW')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('WEBSITE_OVERVIEW')) }}
                    ...
                    "drop view {{ this.database }}.GOOGLE_ANALYTICS.WEBSITE_OVERVIEW_STG"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('WEBSITE_OVERVIEW_STG')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('WEBSITE_OVERVIEW_STG')  }}
    -- WEBSITE_OVERVIEW from {{ source('GOOGLE_ANALYTICS', '_AIRBYTE_RAW_WEBSITE_OVERVIEW') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('WEBSITE_OVERVIEW_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('WEBSITE_OVERVIEW_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('WEBSITE_OVERVIEW_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      lag(DATE) over (
        partition by DATE, PROPERTY_ID, UUID, AVERAGE_SESSION_DURATION
        order by
            DATE is null asc,
            DATE desc,
