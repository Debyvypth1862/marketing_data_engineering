    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('COMMON_STATISTIC')) }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', adapter.quote(this.schema) + '.' + adapter.quote('COMMON_STATISTIC')) }}
                    ...
                    "drop view {{ this.database }}.ALANBASE.COMMON_STATISTIC_STG"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('COMMON_STATISTIC_STG')
with
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('COMMON_STATISTIC_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('COMMON_STATISTIC_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('COMMON_STATISTIC_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      lag(DATE) over (
        partition by CLICKID, TRACKER_LOGIN_ID, DATE
        order by
            DATE is null asc,
            DATE desc,
