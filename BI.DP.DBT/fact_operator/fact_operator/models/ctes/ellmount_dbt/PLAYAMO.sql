...
SELECT
  ...
  END AS SignUp_Date
  , CASE
    WHEN ops."First Time Depositing Customers" > 0 THEN ops.DATE
  END AS FTD_Date
  ...
  , ops.SUBID AS ClickId
  , 1 AS Click_Cnt
  , ops.SIGNUPS AS Signup_Cnt
  , ops."First Time Depositing Customers" AS FTD_Cnt
  , COALESCE(CASE
    WHEN ops."First Time Depositing Customers" > 0 THEN ops.DEPOSITS
  END, 0) AS FTD_Amt
