...
WITH BRC_CLICKS AS (
  SELECT
    pstbk.POST_CLICK_DATE AS Click_Date
  ...
  WHERE
    1 = 1
    {{ incremental_clause_with_custom_column('CLICK_DATE', this, 'DATE(pstbk.POST_CLICK_DATE)') }}
  GROUP BY ALL
)
...
, OPERATOR_CONSOLIDATION AS (
  SELECT
    COALESCE(reg.Click_Date, ftd.Click_Date, rev.Click_Date, brc.Click_Date, cpa.Click_Date) AS Click_Date
  ...
  FULL OUTER JOIN OPERATOR_FIRSTDEPOSIT AS ftd
    ON reg.Click_Date = ftd.Click_Date AND reg.ClickID = ftd.ClickID
  FULL OUTER JOIN OPERATOR_CPA AS cpa
    ON reg.Click_Date = cpa.Click_Date AND reg.ClickID = cpa.ClickID
  FULL OUTER JOIN OPERATOR_REVENUE AS rev
    ON reg.Click_Date = rev.Click_Date AND reg.ClickID = rev.ClickID
  LEFT OUTER JOIN BRC_CLICKS AS brc
    ON reg.Click_Date = ftd.Click_Date AND UPPER(reg.ClickID) = UPPER(brc.ClickID)
  GROUP BY ALL
)
...
, final_cte AS (
  ...
  LEFT OUTER JOIN BRC_CLICKS AS brc
    ON ops.Click_Date = brc.Click_Date AND ops.ClickID = brc.ClickID
  LEFT OUTER JOIN {{ source('BRC', 'POSTBACK_TRACKING') }} AS pstbk
...
, final_cte_dedup AS (
  SELECT
    *
    , ROW_NUMBER() OVER (
        PARTITION BY Click_Date, PUBLISHER_NAME, ADVERTISER_NAME, BRAND_NAME, COUNTRY, Campaign_Name, ClickID
