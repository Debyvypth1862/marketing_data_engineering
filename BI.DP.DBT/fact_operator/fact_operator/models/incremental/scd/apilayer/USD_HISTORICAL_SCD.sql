{{ config(
    ...
    post_hook = ["
                    ...
                    "drop view {{ ref('USD_HISTORICAL_STG') }}"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('USD_HISTORICAL_STG')
WITH
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('USD_HISTORICAL_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('USD_HISTORICAL_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('USD_HISTORICAL_STG')) }} from previous_active_scd_data
),
{% else %}
  input_data AS (
    SELECT *
