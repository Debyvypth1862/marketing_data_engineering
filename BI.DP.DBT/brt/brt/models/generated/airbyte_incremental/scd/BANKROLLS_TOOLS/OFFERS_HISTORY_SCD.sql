{{ config(
    ...
    post_hook = ["
                    ...
                    FROM (SELECT S3_PATH, count(*) as RECORD_COUNT
                     ...
                     "drop view {{ this.database }}.BRT.OFFERS_HISTORY_STG"],
                    
    tags = [ "top-level" ]
) }}
-- depends_on: ref('OFFERS_HISTORY_STG')
with
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('OFFERS_HISTORY_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('OFFERS_HISTORY_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('OFFERS_HISTORY_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
