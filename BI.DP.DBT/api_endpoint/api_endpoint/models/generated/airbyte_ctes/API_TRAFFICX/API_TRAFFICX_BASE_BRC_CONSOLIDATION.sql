...
-- BRC Consolidation for trafficx
Select
...
from {{ ref('API_TRAFFICX_BASE_BRC_CLICKS') }} brc
full outer join {{ ref('API_TRAFFICX_BASE_BRC_REGISTRATION') }} reg
    on brc.ClickID = reg.ClickID
full outer join {{ ref('API_TRAFFICX_BASE_BRC_FIRST_DEPOSIT') }} ftd
    on brc.clickid = ftd.clickid
full outer join {{ ref('API_TRAFFICX_BASE_BRC_CPA') }} cpa
