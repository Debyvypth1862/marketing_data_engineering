...
-- Final Consolidation for trafficx
Select
...
from {{ ref('API_TRAFFICX_BASE_BRC_CONSOLIDATION') }} brc
full outer join {{ ref('API_TRAFFICX_BASE_OPS_CONSOLIDATION') }} reg
    on upper(brc.ClickID) = upper(reg.ClickID) and brc.Date = reg.Date
full outer join {{ ref('API_TRAFFICX_BASE_OPS_REVSHARE') }} rev
