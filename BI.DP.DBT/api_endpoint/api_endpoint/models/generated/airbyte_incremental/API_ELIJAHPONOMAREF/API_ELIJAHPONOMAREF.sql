...
-- Final table for API_ELIJAHPONOMAREF with created_at/modified_at
-- Reads from SCD table and adds user-friendly timestamp columns
-- depends_on: {{ ref('API_ELIJAHPONOMAREF_SCD') }}

with scd_active as (
    select
        *
    from {{ ref('API_ELIJAHPONOMAREF_SCD') }}
...
first_version as (
    select
        ...
        min(_AIRBYTE_API_ELIJAHPONOMAREF_HASHID) as first_hash
    from {{ ref('API_ELIJAHPONOMAREF_SCD') }}
...
select
    ...
    scd._AIRBYTE_API_ELIJAHPONOMAREF_HASHID,
    ...
    case
        when scd._AIRBYTE_API_ELIJAHPONOMAREF_HASHID = fv.first_hash then null
