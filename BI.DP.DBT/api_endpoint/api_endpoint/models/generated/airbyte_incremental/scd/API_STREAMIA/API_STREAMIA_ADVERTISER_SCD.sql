{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema='API_ENDPOINT',
                            identifier='API_STREAMIA_ADVERTISER'
                        )
                    %}
                    {%
                    ...
                    {% endif %}
                    ",
                    "drop view {{ env_var('INTM_DATABASE', 'INTM') }}.API_ENDPOINT.API_STREAMIA_ADVERTISER_STG"],
    tags = [ "top-level" ]
) }}
-- SCD Type 2 model for API_STREAMIA_ADVERTISER
-- depends_on: {{ ref('API_STREAMIA_ADVERTISER_STG') }}
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('API_STREAMIA_ADVERTISER_STG')  }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('API_STREAMIA_ADVERTISER_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('API_STREAMIA_ADVERTISER_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('API_STREAMIA_ADVERTISER_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      lag(_AIRBYTE_EMITTED_AT) over (
        partition by DATE, ADVERTISER_ID, AFFILIATE_ID
        order by
            DATE is null asc,
            DATE desc,
      ...
      _AIRBYTE_AB_ID,
      _AIRBYTE_EMITTED_AT,
      _AIRBYTE_API_STREAMIA_ADVERTISER_HASHID
    from input_data
),
dedup_data as (
    select
