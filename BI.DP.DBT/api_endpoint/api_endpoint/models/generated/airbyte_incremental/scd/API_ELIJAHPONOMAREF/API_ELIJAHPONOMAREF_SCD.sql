{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema='API_ENDPOINT',
                            identifier='API_ELIJAHPONOMAREF'
                        )
                    %}
                    {%
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._AIRBYTE_UNIQUE_KEY in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', env_var('EXP_DATABASE', 'EXP') ~ '.PUBLIC.API_ELIJAHPONOMAREF') }}
                            ) recent_records
                            left join (
                                select _AIRBYTE_UNIQUE_KEY as unique_key, count(_AIRBYTE_UNIQUE_KEY) as active_count
                                from {{ this }}
                                where _AIRBYTE_ACTIVE_ROW = 1 {{ incremental_clause('_AIRBYTE_NORMALIZED_AT', env_var('EXP_DATABASE', 'EXP') ~ '.PUBLIC.API_ELIJAHPONOMAREF') }}
                    ...
                    {% endif %}
                    ",
                    "drop view {{ env_var('INTM_DATABASE', 'INTM') }}.API_ENDPOINT.API_ELIJAHPONOMAREF_STG"],
    tags = [ "top-level" ]
) }}
-- SCD Type 2 model for API_ELIJAHPONOMAREF
-- depends_on: {{ ref('API_ELIJAHPONOMAREF_STG') }}
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('API_ELIJAHPONOMAREF_STG')  }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('API_ELIJAHPONOMAREF_STG'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('API_ELIJAHPONOMAREF_STG')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('API_ELIJAHPONOMAREF_STG')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      _AIRBYTE_AB_ID,
      _AIRBYTE_EMITTED_AT,
      _AIRBYTE_API_ELIJAHPONOMAREF_HASHID
    from input_data
),
dedup_data as (
    select
